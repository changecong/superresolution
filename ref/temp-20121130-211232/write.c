 ...
 * Creation Date: [2012-12-01 03:58]
 * Last Modified: [2012-12-01 07:53]
...
/* -- Basic I/O */
int ofp_ptr=0;
unsigned int ofp[512];

// table to mask the n lowest bits 
unsigned int lmask[] = {
		0x0000, 
...
void WriteSOF(unsigned int imagewidth, unsigned int imageheight)
{
	int end_p, start_p;
...
void genBmp(double avim[H_IMG_HEIGHT][H_IMG_WIDTH]) 
{
   input = avim;
	...
	WriteMarker(0xd8);
    WriteAPP0();
	WriteSOF(H_IMG_WIDTH, H_IMG_HEIGHT);
	WriteDQT();
	WriteDHT();
	WriteSOS();
    ...
    for (i = 0; i < H_IMG_HEIGHT; i++) {
      for (j = 0; j < H_IMG_WIDTH; j++) {
        ofp[ofp_ptr++] = (int)((avim[i][j]+0.5)*255);
      }
      FileWrite(ofp, ofp_ptr);
      ofp_ptr = 0;
    }
